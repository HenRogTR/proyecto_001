...
<%
    ...
    try {
        ...
        if (tipo.equals("todo")) {
            lCobranza = objcCobranza.leer_todo_fechaInicio_fechaFin(fechaInicio, fechaFin);
        }
        if (tipo.equals("iniciales")) {
            lCobranza = objcCobranza.leer_iniciales_fechaInicio_fechaFin(fechaInicio, fechaFin);
        }
        if (tipo.equals("cobranza")) {
            lCobranza = objcCobranza.leer_cobranza_fechaInicio_fechaFin(fechaInicio, fechaFin);
        }
        if (tipo.equals("anticipos")) {
            lCobranza = objcCobranza.leer_anticipo_fechaInicio_fechaFin(fechaInicio, fechaFin);
        }
        if (lCobranza == null) {
        ...
        <div id="documento">
            <div id="contenido">
                <table style="font-size: 11px; width: 800px;">                    
                    <tr class="bottom2">
                        <th style="text-align: left;" colspan="3">REPORTE DE COBRANZA - <%=objcUtilitarios.fechaDateToString(fechaInicio)%> a <%=objcUtilitarios.fechaDateToString(fechaFin)%> - <%=tipo.toUpperCase()%></th>                
                    ...
                    <%
                        ...
                        while (i.hasNext()) {
                            ...
                            if (objCobranza.getRegistro().substring(0, 1).equals("0") || objCobranza.getRegistro().substring(0, 1).equals("1")) {
                                if (con++ == 0) {
