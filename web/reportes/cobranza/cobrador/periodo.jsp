<%-- 
    Document   : periodo
    Created on : 07/06/2013, 10:09:24 PM
    Author     : Henrri
--%>
...
<%
    ...
    int codCobrador = 0;
    //clases
    Persona objCobrador = new Persona();
    ...
    try {
        ...
        if (tipo.equals("todo")) {
            lCobranza = objcCobranza.leer_todo_fechaInicio_fechaFin_codCobrador(fechaInicio, fechaFin, codCobrador);
        }
        if (tipo.equals("iniciales")) {
            lCobranza = objcCobranza.leer_iniciales_fechaInicio_fechaFin_codCobrador(fechaInicio, fechaFin, codCobrador);
        }
        if (tipo.equals("cobranza")) {
            lCobranza = objcCobranza.leer_cobranza_fechaInicio_fechaFin_codCobrador(fechaInicio, fechaFin, codCobrador);
        }
        if (tipo.equals("anticipos")) {
            lCobranza = objcCobranza.leer_anticipo_fechaInicio_fechaFin_codCobrador(fechaInicio, fechaFin, codCobrador);
        }
        if (lCobranza == null) {
        ...
        <div id="documento">
            <div id="contenido">
                <table style="font-size: 11px; width: 800px;">
                    <tr class="bottom2">
                        <th style="text-align: left;" colspan="3">REPORTE DE COBRANZA - <%=objcUtilitarios.fechaDateToString(fechaInicio)%> a <%=objcUtilitarios.fechaDateToString(fechaFin)%> - <%=tipo.toUpperCase()%> (C. <%=objCobrador.getNombres()%>)</th>                
