<%-- 
    Document   : ventasAnuladosExcel
    Created on : 05/07/2013, 11:33:17 AM
    Author     : Henrri
--%>
...
<%
    ...
    try {
        ...
        if (ventaTipo.equals("todo")) {
            lVentas = objcVentas.leer_fechaInicio_fechaFin_anulado(fechaInicio, fechaFin);
        }
        if (ventaTipo.equals("contado")) {
            lVentas = objcVentas.leer_contado_fechaInicio_fechaFin_anulado(fechaInicio, fechaFin);
        }
        if (ventaTipo.equals("credito")) {
            lVentas = objcVentas.leer_credito_fechaInicio_fechaFin_anulado(fechaInicio, fechaFin);
        }
        if (lVentas == null) {
            estado = false;
        ...
        <div id="documento">
            <div id="contenido">
                ...
                <table style="width: 800px; font-size: 14px; ">
                    <thead>
                        <tr class="bottom2">
                            <th colspan="5">REPORTE GENERAL DE VENTAS(<%=ventaTipo%>) <%=objcUtilitarios.fechaDateToString(fechaInicio) + " - " + objcUtilitarios.fechaDateToString(fechaFin)%></th>
                        </tr>
