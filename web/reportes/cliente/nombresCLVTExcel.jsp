...
<%
    ...
    try {
        ...
        LVList = new cVentaCreditoLetra().letras_ordenNombresC();
        if (LVList == null) {
    ...
    response.setHeader("Content-Disposition", "attachment;filename=\"TRAMOS (APE/NOM) " + fecha + " " + objcManejoFechas.fechaHoraActualNumerosLineal() + ".xls\"");
%>
...
<html>
    <head>
        ...
        <title>TRAMOS (APE/NOM) <%=fecha%> <%=objcManejoFechas.fechaHoraActualNumerosLineal()%></title>
        <style>
            td,th{
                text-align: left;
                vertical-align: top;
    ...
    <body>
        <table style="font-size: 14px;width: 2100px;">
            <thead>
                <tr class="bottom2">
                    <th colspan="11"><label>Letras X cobrar : Clientes en general (ape/nom)</label></th>
                </tr>
