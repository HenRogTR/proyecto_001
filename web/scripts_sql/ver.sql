...
-- -----------------------------------------------------
-- Table `proyecto_001`.`venta_cuota_pago`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `proyecto_001`.`venta_cuota_pago` ;

CREATE TABLE IF NOT EXISTS `proyecto_001`.`venta_cuota_pago` (
  `cod_venta_cuota_pago` INT NOT NULL AUTO_INCREMENT,
  `ventas_cod_ventas` INT(11) NOT NULL,
  `numero_letra` INT(11) NOT NULL DEFAULT 0,
  `detalle_letra` VARCHAR(45) CHARACTER SET 'utf8' COLLATE 'utf8_spanish_ci' NULL DEFAULT NULL,
  `fecha_vencimiento` DATETIME NULL DEFAULT NULL,
  `monto` DOUBLE(10,2) NOT NULL DEFAULT '0.00',
  `interes` DOUBLE(10,2) NOT NULL DEFAULT 0.00,
  `fecha_pago` DATETIME NULL,
  `total_pago` DOUBLE(10,2) NOT NULL DEFAULT 0.00,
  `interes_pagado` DOUBLE(10,2) NOT NULL DEFAULT 0.00,
  `interes_pendiente` DOUBLE(10,2) NOT NULL DEFAULT 0.00,
  `interes_ultimo_calculo` DATE NULL,
  `registro` TEXT CHARACTER SET 'utf8' COLLATE 'utf8_spanish_ci' NOT NULL,
  PRIMARY KEY (`cod_venta_cuota_pago`),
  INDEX `fk_venta_cuota_pago_ventas1_idx` (`ventas_cod_ventas` ASC),
  CONSTRAINT `fk_venta_cuota_pago_ventas1`
    FOREIGN KEY (`ventas_cod_ventas`)
    REFERENCES `proyecto_001`.`ventas` (`cod_ventas`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
